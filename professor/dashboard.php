...
<style>
:root{--brand-1:#0f766e;--brand-2:#0ea5e9;--accent:#06b6d4;}
body {display:flex;min-height:100vh;margin:0;background:#f8f9fa;}
.sidebar{background: linear-gradient(180deg,#007bff,#28a745);color:white;min-height:100vh;position:fixed;width:250px;transition:0.3s;z-index:1000;}
...
.sidebar-toggle{position:fixed;top:15px;left:15px;z-index:1100;background:#007bff;color:white;border-radius:50%;width:45px;height:45px;display:flex;align-items:center;justify-content:center;}
@media(max-width:768px){.sidebar{left:-250px;}.sidebar.collapsed{left:0;}main{margin-left:0;width:100%;}}
...
.user{background:#fff7;border:1px solid rgba(0,0,0,0.05);}
...
.slot{cursor:pointer;border-radius:8px;padding:6px 10px;margin:4px;display:inline-block;border:1px dashed rgba(0,0,0,0.08);}
.slot.selected{background:var(--brand-2);color:white;}
    ...
    <div id="chatArea">
      ...
      <div id="step-student" class="step-hidden">
        <div class="chat-bubble p-3">
          ...
          <input id="student_id" type="text" class="form-control mb-2" placeholder="2022-12345">
          <label class="form-label">First name</label>
          <input id="student_first" class="form-control mb-2">
          <label class="form-label">Middle name</label>
          <input id="student_m" class="form-control mb-2">
          <label class="form-label">Last name</label>
          <input id="student_last" class="form-control mb-2">
          <label class="form-label">Course</label>
          <input id="student_course" class="form-control mb-2">
          <label class="form-label">Email</label>
          <input id="student_email" type="email" class="form-control mb-2" placeholder="student@example.com">
      ...
      <div id="step-college" class="step-hidden">
        <div class="chat-bubble p-3">
          ...
          <select id="college_select" class="form-select mb-2">
          ...
          <select id="facilitator_select" class="form-select mb-2" disabled>
      ...
      <div id="step-schedule" class="step-hidden">
        <div class="chat-bubble p-3">
          <div class="mb-2"><strong>Available schedule slots</strong></div>
          <div id="slots_container" class="mb-2">Choose a facilitator to load slots.</div>
          <label class="form-label">Purpose / Reason</label>
          <textarea id="reason" class="form-control mb-2" rows="2"></textarea>
          <div class="d-flex justify-content-between mt-2">
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  sidebarToggle.addEventListener('click', ()=>{sidebar.classList.toggle('collapsed');main.classList.toggle('full-width');});

  // Step navigation
  q('#toCollege').addEventListener('click', ()=>{
    if(!q('#student_id').value.trim()||!q('#student_first').value.trim()||!q('#student_last').value.trim()||!q('#student_email').value.trim()){alert('Please fill all required student info.');return;}
  ...
  q('#college_select').addEventListener('change', async()=>{
    ...
    const sel=q('#facilitator_select'); sel.innerHTML='<option>Loading...</option>'; sel.disabled=true; q('#toSchedule').disabled=true;
    ...
    try{
      ...
      data.forEach(f=>{ const opt=document.createElement('option'); opt.value=f.facilitator_id; opt.textContent=f.first_name+' '+f.last_name; sel.appendChild(opt); });
      sel.disabled=false;
  ...
  q('#submitReferral').addEventListener('click', async()=>{
    const chosen=document.querySelector('.slot.selected'); if(!chosen){alert('Please choose a slot.');return;}
    const payload={student_id:q('#student_id').value.trim(), student_first:q('#student_first').value.trim(), student_m:q('#student_m').value.trim(), student_last:q('#student_last').value.trim(), student_course:q('#student_course').value.trim(), student_email:q('#student_email').value.trim(), college_id:q('#college_select').value, facilitator_id:q('#facilitator_select').value, reason:q('#reason').value.trim(), appointment_date:chosen.dataset.date, appointment_time:chosen.dataset.time};
    ...
    try{
      ...
      if(!resp.ok){const t=await resp.text();console.error('Server:',t); alert('Server error'); q('#submitReferral').disabled=false; return;}
      const j=await resp.json();
      if(j.status==='success'){hide('step-schedule'); show('successBox'); q('#successMessage').innerHTML=`<p>Referral ID <b>${j.referral_id}</b>, Appointment <b>${payload.appointment_date} ${payload.appointment_time}</b> (ID <b>${j.appointment_id}</b>)</p>`;}
      else{alert(j.message||'Error'); q('#submitReferral').disabled=false;}
...
async function loadSlotsForFacilitator(fid){
  ...
  try{
    ...
    const grouped={};
    slots.forEach(s=>{if(!grouped[s.date])grouped[s.date]=[];grouped[s.date].push(s.time);});
    container.innerHTML='';
    Object.keys(grouped).sort().forEach(date=>{
      const g=document.createElement('div'); g.className='date-group';
      const h=document.createElement('div'); h.innerHTML=`<strong>${new Date(date).toLocaleDateString('en-US',{weekday:'short',month:'short',day:'numeric'})}</strong>`; g.appendChild(h);
      grouped[date].sort().forEach(time=>{
        const el=document.createElement('div'); el.className='slot'; el.dataset.date=date; el.dataset.time=time; el.textContent=new Date('1970-01-01T'+time).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'});
        el.addEventListener('click', ()=>{document.querySelectorAll('.slot').forEach(x=>x.classList.remove('selected')); el.classList.add('selected'); q('#submitReferral').disabled=false;});
        g.appendChild(el);
      });
